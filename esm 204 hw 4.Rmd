...
Present value of damages under Policy A: `r round(ev_a/1e+9, 0)` billion
Present value of damages under Policy B: `r round(ev_b/1e+9, 0)` billion

```{r}
# cost sequence
cost <- seq(0, 5e15, by = 1e9) 
...
# graph of damages under policy scenarios
ggplot(data = cost_compare) +
  geom_line(aes(x = cost/1e9, y = cost_a/1e9), color = "darkseagreen") + 
  geom_line(aes(x = cost/1e9, y = cost_b/1e9), color = "indianred") +
  theme_minimal() + 
  labs(x = "Implementation Cost (billions USD)", y = "Expected Value of Cost (billions USD)") 
```
